...
public:
    ...
    bool intersect_1(float a, float b, float c, float d) { //Вспомогательная функция для персечения
        if (a > b)  swap(a, b);
        if (c > d)  swap(c, d);
        return max(a, c) <= min(b, d);
    }

    bool Cross_B(Segment n) { //Альтернативная функция пересечения отрезков
    ...
    bool Cross(Segment n, Segment m, point c) { //Пересечение отрезков при помощи векторов
        float k;
    ...
    bool Point_Cross(point a) { //Проверка принадлежности точки к отрезку
